...
@php
    $kt_logo_image = 'logo-4.png';
@endphp

@if (config('layout.brand.self.theme') === 'light')
    @php $kt_logo_image = 'logo-4.png' @endphp
...
<div class="aside aside-left {{ Metronic::printClasses('aside', false) }} d-flex flex-column flex-row-auto" id="kt_aside">
    ...
    <div class="brand flex-column-auto {{ Metronic::printClasses('brand', false) }}" id="kt_brand">
        <div class="brand-logo">
            <a href="{{ url('/') }}">
                <img alt="{{ config('app.name') }}" src="{{ asset('media/logos/'.$kt_logo_image) }}" width="50%"/>
            </a>
        </div>
    ...
    <div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper">
        ...
        <div
            ...
            <ul class="menu-nav {{ Metronic::printClasses('aside_menu_nav', false) }}">
                ...
                @if(auth('admin')->check())
                    {{ \App\Classes\Theme\Menu::renderVerMenu(config('menu_aside.admin-items'))}}
                @elseif(auth()->check())
                    {{ \App\Classes\Theme\Menu::renderVerMenu(config('menu_aside.company-items'))}}
                @else
                @endif
